      ...
      <div className="w-full flex items-center justify-center">
        <div className="container px-4 lg:px-0">
          <div className="grid grid-cols-1 lg:grid-cols-4 mt-5 gap-6">
            <div>
              <div className="  shadow_custom py-10 rounded-md">
                <div className=" h-40 w-full flex items-center justify-center">
                  <img
                    src={fakedata[0].data[0].credits[1].user.avatarUrl}
                ...
                <h1
                  style={{ color: "#E36948" }}
                  className="  text-3xl font-medium text-center my-2"
                >
                  {fakedata[0].data[0].credits[1].user.name}
                </h1>
                <div className=" flex w-full items-center justify-center gap-3 text-gray-600">
                  <FaTwitter className=" w-5 h-5 cursor-pointer" />
                  <AiOutlineInstagram className=" w-5 h-5 cursor-pointer" />
                  <FaFacebookSquare className=" w-5 h-5 cursor-pointer" />
                  <SiTiktok className=" w-5 h-5 cursor-pointer" />
                </div>

                <div className=" px-5  py-3">
                  <p className=" text-lg font-light ">
                    Photographer and Editor based in Miami, FL. Lover of God and
                    People. @UniversityofKY Grad.
                  </p>
                  <p className=" text-lg font-light">#JKru</p>
                </div>
                <div className=" py-3 px-5">
                  <div className=" flex items-center justify-between">
                    <h6 className=" font-medium text-lg">Total Posts</h6>
                    <p className=" font-thin">
                      {fakedata[0].meta.stats.totalPosts}
                    </p>
                  </div>
                  <div className="flex items-center justify-between">
                    <h6 className=" font-medium text-lg">Total Views</h6>
                    <p className=" font-thin">
                      {fakedata[0].meta.stats.totalViews}
                    </p>
                  </div>
                  <div className="flex items-center justify-between">
                    <h6 className=" font-medium text-lg">Total Likes</h6>
                    <p className=" font-thin">
                      {fakedata[0].meta.stats.totalLikes}
                    </p>
                  </div>
                  <div className="flex items-center justify-between">
                    <h6 className=" font-medium text-lg">Total Shares</h6>
                    <p className=" font-thin">
                      {fakedata[0].meta.stats.totalShares}
                    </p>
                  </div>
                  <div className="flex items-center justify-between">
                    <h6 className=" font-medium text-lg">Total Comments</h6>
                    <p className=" font-thin">
                      {fakedata[0].meta.stats.totalComments}
                    </p>
                  </div>
                </div>
                <h1 className=" text-center font-medium text-lg pt-3">
                  TOP COLLABORATORS
                </h1>
                <div className=" w-full px-8 pt-3 grid grid-cols-5 gap-2">
                  {collaborators.data.map((item, ind) => (
                    <Tooltip title={item.name} key={ind} placement="top">
                      <img
                        src={item.avatarUrl}
            ...
            <div className=" lg:col-span-3">
              ...
              <div className=" grid grid-cols-1 lg:grid-cols-3 gap-5">
                <div className="flex flex-col gap-5">
                  {fakedata[0].data.slice(0, 8).map((item, ind) => (
                    <CardComponent item={item} key={ind} />
                  ))}
                </div>
                <div className="flex flex-col gap-5">
                  {fakedata[0].data.slice(9, 17).map((item, ind) => (
                    <CardComponent item={item} key={ind} />
                  ))}
                </div>
                <div className="flex flex-col gap-5">
                  {fakedata[0].data.slice(18, 25).map((item, ind) => (
                    <CardComponent item={item} key={ind} />
                  ))}
                </div>
              </div>
